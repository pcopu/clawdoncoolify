...
**Recommended (simplest):** Dockerfile app.

1) Create a new **Dockerfile** app in Coolify and point it at this repo.
2) Set the app port to **18789** (auto‑detected from `EXPOSE`).
3) Add at least one model provider key (see below).
4) Deploy.

**Alternative:** Docker Compose app (advanced).
1) Create a new **Docker Compose** app in Coolify and point it at this repo.
2) Coolify will map port **18789** from `docker-compose.yaml`.
3) Add at least one model provider key (see below).
4) Deploy.
...
- `PORT` — optional; if set, overrides the gateway port (useful for PaaS)
